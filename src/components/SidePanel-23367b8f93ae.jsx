...
import { Battery, Coins, Ticket, Dices, ArrowRight } from 'lucide-react';
...
const SidePanel = () => {
    ...
    return (
        <div className="flex flex-col h-full p-4">
            {/* Header */}
            <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-purple-500 mb-4 tracking-wider">
                外卖势力大富翁
            </h1>
            ...
            {activePlayer && (
                <div className="bg-slate-800 p-4 rounded-xl border border-slate-600 mb-4">
                    ...
                    <div className="flex flex-col gap-2">
                        {/* Active Skill Area */}
                        {(() => {
                            const skill = activePlayer.stats.faction.activeSkill;
                            const [targetMode, setTargetMode] = React.useState(false);

                            // Reset target mode when player changes
                            React.useEffect(() => {
                                setTargetMode(false);
                                ...
                                <div className="mb-3 p-2 bg-slate-900/80 rounded border border-purple-500/30">
                                    <div className="flex justify-between items-center mb-1">
                                        <span className="text-xs font-bold text-purple-400">
                                            ⚡ 专属技能: {skill.id === 'pause_enemy' ? '强制暂停' : '电能转化'}
                                        </span>
                                    ...
                                    <div className="text-[10px] text-slate-400 mb-2 leading-tight">{skill.desc}</div>

                                    {skill.id === 'pause_enemy' ? (
                                        !targetMode ? (
                                            <button
                                                disabled={activePlayer.skillCooldown > 0 || state.phase !== 'IDLE' || activePlayer.money < skill.cost}
                                            ...
                                            <div className="flex flex-col gap-1 animate-in fade-in slide-in-from-top-2">
                                                ...
                                                <div className="grid grid-cols-2 gap-1">
                                                    {state.players.map((p, idx) => {
                                                        if (idx === state.currentPlayerIndex) return null;
                                                        return (
                                                            <button
                                                                key={idx}
                                                                onClick={() => {
                                        ...
                                        // Eleme
                                        <button
                                            disabled={activePlayer.skillCooldown > 0 || state.phase !== 'IDLE' || activePlayer.power < skill.costPower}
                        ...
                        {/* Log */}
                        <div className="mt-4 h-32 overflow-y-auto text-xs font-mono text-slate-400 bg-black p-2 rounded">
